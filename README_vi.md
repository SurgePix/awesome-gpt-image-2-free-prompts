<div align="center">

<img src="https://udefineus-ui-prod-1316642525.cos.ap-seoul.myqcloud.com/surgepix-resources/present/vi-banner.png" alt="awesome-gpt-image-2-free-prompts" style="max-width: 100%;" />

</div>

# 🎨 Awesome GPT-Image-2 Prompt Miễn Phí
[![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](LICENSE)
[![PRs Welcome](https://img.shields.io/badge/PRs-welcome-brightgreen.svg)](CONTRIBUTING.md)
[![GitHub stars](https://img.shields.io/github/stars/SurgePix/awesome-gpt-image-2-free-prompts?style=social)](https://github.com/SurgePix/awesome-gpt-image-2-free-prompts)

**Thư viện prompt GPT-Image-2 miễn phí toàn diện nhất**

Prompt được tuyển chọn · Ảnh xem trước · Sao chép & tạo ảnh 1 cú nhấp · Đa ngôn ngữ · Cập nhật thường xuyên

[![Prompts](https://img.shields.io/badge/Prompts-367%2B-ff6b6b?style=flat-square)](https://surgepix.ai/resources/gpt-image-2-prompts)
[![Weekly Updated](https://img.shields.io/badge/Cập%20nhật-Hàng%20tuần-4ecdc4?style=flat-square)](#)
[![Languages](https://img.shields.io/badge/Ngôn%20ngữ-6%2B-45b7d1?style=flat-square)](#-hỗ-trợ-đa-ngôn-ngữ)
[![License](https://img.shields.io/badge/Giấy%20phép-MIT-a8e6cf?style=flat-square)](LICENSE)
[![Website](https://img.shields.io/badge/Website-surgepix.ai-ff8b94?style=flat-square)](https://surgepix.ai/resources/gpt-image-2-prompts)

[![EN](https://img.shields.io/badge/EN-English-blue?style=flat-square)](README.md)
[![ZH](https://img.shields.io/badge/ZH-%E4%B8%AD%E6%96%87-red?style=flat-square)](README_zh.md)
[![TR](https://img.shields.io/badge/TR-T%C3%BCrk%C3%A7e-orange?style=flat-square)](README_tr.md)
[![VI](https://img.shields.io/badge/VI-Ti%E1%BA%BFng%20Vi%E1%BB%87t-green?style=flat-square)](README_vi.md)
[![PL](https://img.shields.io/badge/PL-Polski-purple?style=flat-square)](README_pl.md)
[![ID](https://img.shields.io/badge/ID-Indonesia-teal?style=flat-square)](README_id.md)

> Thư viện **prompt GPT Image 2** miễn phí lớn nhất — bao gồm chân thực,
> typography, UI/UX, ảnh chụp màn hình game, thiết kế nhân vật và minh họa thương mại.
> 500+ prompt sẵn sàng sử dụng, hỗ trợ nhiều ngôn ngữ.

## Mục lục

- [Tính năng](#features)
- [Danh mục](#categories)
- [Prompt Nổi bật](#featured-prompts)
- [Hỗ trợ đa ngôn ngữ](#multilingual-support)
- [Cách sử dụng](#how-to-use)
- [Truy xuất nguồn gốc](#source-tracing)
- [Về GPT-Image-2](#about-gpt-image-2)
- [Đóng góp](CONTRIBUTING.md)

[**→ Xem Toàn Bộ Thư Viện**](https://surgepix.ai/resources/vi/gpt-image-2-prompts)

[![GPT-Image-2 Prompt Library Preview](https://udefineus-ui-prod-1316642525.cos.ap-seoul.myqcloud.com/surgepix-resources/present/vi-gpt-image-2-all-prompt.png)](https://surgepix.ai/resources/vi/gpt-image-2-prompts)

---

## ✨ Tính Năng

| Tính năng | Mô tả |
|---|---|
| 🖼️ **Ảnh xem trước** | Mỗi prompt đi kèm ảnh AI thực tế được tạo ra |
| 📋 **Sao chép 1 cú nhấp** | Sao chép prompt vào clipboard ngay lập tức |
| ⚡ **Tạo ảnh trực tiếp** | Chuyển sang SurgePix để tạo ảnh bằng prompt |
| 🌍 **Đa ngôn ngữ** | Prompt và giao diện hỗ trợ nhiều ngôn ngữ |
| 🔍 **Lọc theo danh mục** | Duyệt theo phong cách, mục đích hoặc loại nội dung |
| 🔗 **Truy nguồn gốc** | Mỗi prompt cộng đồng đều liên kết đến bài đăng gốc trên mạng xã hội |
| 📅 **Cập nhật thường xuyên** | Thêm prompt mới mỗi ngày từ các nhà sáng tạo hàng đầu |
| 🆓 **Hoàn toàn miễn phí** | Không cần đăng nhập, không có paywall |

---

## 🗂️ Danh Mục

| 🖼️ OpenAI Chính thức | 🎨 Poster / Anime | 📐 Lưới / Ghép ảnh | 📸 Nhiếp ảnh |
|---|---|---|---|
| 👤 Chân dung | 💻 UI / UX | 🏛️ Kiến trúc | 📦 Sản phẩm |
| ✏️ Minh họa | 🎄 Lễ hội / Kỳ nghỉ | 🎭 Nhân vật | ➕ Thêm thường xuyên |

<details>
<summary><b>🖼️ OpenAI Chính Thức</b> — Prompt từ các bản phát hành chính thức của OpenAI</summary>

- **Phong cách Anime** — Chuyển đổi bất kỳ ảnh nào thành nghệ thuật anime xu hướng với cel shading và tỷ lệ phóng đại
- **Poster Bản Vẽ Kỹ Thuật** — Render đối tượng thành sơ đồ đường kẻ trắng kỹ thuật trên nền xanh cobalt đậm
- **Thước Phim** — Biến ảnh thành 3 khung phim điện ảnh tuần tự với hạt phim analog
- **Truyện Tranh** — Tái hiện cảnh quay thành phong cách truyện tranh Mỹ retro với kết cấu halftone
- **Lưới Thiết Kế Icon** — Trừu tượng hóa đối tượng thành 16–20 biến thể logo vector tối giản
- **Lá Bài Tarot** — Tạo lá bài tarot cá nhân hóa theo phong cách vẽ tay Rider-Waite cổ điển
- **Avatar 3D** — Tạo render đồ chơi vinyl bóng loáng với tóc điêu khắc và kính mát retro
- **Thực Vật Học Vintage** — Diễn giải lại đối tượng theo phong cách minh họa atlas khoa học thế kỷ 19

</details>

<details>
<summary><b>🎄 Ngày của Mẹ / Lễ Hội</b> — Thiệp chúc mừng theo mùa và nghệ thuật lễ hội</summary>

- **Thiệp Nữ Hoàng Cơ Đỏ** — Lá bài phong cách gouache với khuôn mặt mẹ thay thế nữ hoàng
- **Thiệp Minh Họa Phẳng** — Poster lá bài đơn sắc hồng-đỏ tối giản
- **Ảnh Hoa Cẩm Chướng** — Thiệp thực tế với hoa cẩm chướng bokeh mềm và không gian viết tay
- **Thiệp Hoa Tulip Matisse** — Bố cục hoa tulip phẳng trừu tượng lấy cảm hứng từ kỹ thuật cắt giấy của Matisse
- **Hình Vẽ Đường Liên Tục** — Chân dung mẹ và con một nét vẽ, kết hợp họa tiết hoa hồng

</details>

<details>
<summary><b>🎨 Poster / Anime</b> — Nghệ thuật nhân vật cách điệu và poster đồ họa</summary>

- Chuyển đổi nhân vật anime từ ảnh tham chiếu
- Thiết kế poster retro với typography đậm
- Poster sự kiện minh họa phẳng với bảng màu tùy chỉnh

</details>

<details>
<summary><b>📐 Lưới / Ghép Ảnh</b> — Bố cục đa bảng và thiết kế tổng hợp</summary>

- Bảng ghép mood board 4 ô
- Bố cục lưới kể chuyện tuần tự
- Lưới so sánh trước/sau

</details>

<details>
<summary><b>📸 Nhiếp Ảnh</b> — Phong cách ảnh điện ảnh và editorial</summary>

- **Flash Ban Đêm** — Flash gắn máy ảnh cứng với bóng tối sâu và điểm sáng bóng
- **Phim Hạt Điện Ảnh** — Khung phim tương phản cao tone lạnh với hiệu ứng mờ chuyển động
- **Thiết Kế Nội Thất** — Chuyển đổi phòng phong cách mid-century modern thực tế

</details>

<details>
<summary><b>👤 Chân Dung</b> — Tạo ảnh tập trung vào nhân vật và con người</summary>

- Chân dung anime cách điệu từ ảnh
- Avatar đồ chơi vinyl 3D
- Chân dung lá bài tarot cá nhân hóa

</details>

<details>
<summary><b>💻 UI / UX</b> — Mockup giao diện và hình ảnh thiết kế ứng dụng</summary>

- Mockup màn hình ứng dụng di động
- UI bảng điều khiển và trực quan hóa dữ liệu
- Minh họa wireframe trang đích

</details>

<details>
<summary><b>🏛️ Kiến Trúc</b> — Tòa nhà, không gian và nghệ thuật kết cấu</summary>

- Render kiến trúc ngoại thất
- Chuyển đổi thiết kế nội thất
- Sơ đồ kỹ thuật phong cách blueprint

</details>

<details>
<summary><b>📦 Sản Phẩm</b> — Trực quan hóa sản phẩm thương mại</summary>

- Chụp ảnh sản phẩm với nền sạch
- Mockup thiết kế bao bì
- Lưới logo và nhận diện thương hiệu

</details>

<details>
<summary><b>✏️ Minh Họa</b> — Phong cách nghệ thuật vẽ tay và kỹ thuật số</summary>

- Minh họa khoa học thực vật học vintage
- Nghệ thuật tối giản đường liên tục
- Phong cách kết cấu màu nước và gouache

</details>

---

## 🔥 Prompt Nổi Bật

⭐ Được tuyển chọn kỹ lưỡng vì chất lượng và sáng tạo xuất sắc — [Xem tất cả 367+](https://surgepix.ai/resources/gpt-image-2-prompts)

---

<details>
<summary><b>No.1 · Chuyển Đổi Phong Cách Anime</b></summary>

![EN](https://img.shields.io/badge/EN-blue?style=flat-square) ![OpenAI Official](https://img.shields.io/badge/OpenAI%20Official-orange?style=flat-square) ![Featured](https://img.shields.io/badge/%E2%AD%90%20Featured-yellow?style=flat-square)

**Mô tả:** Chuyển đổi bất kỳ ảnh tải lên thành phong cách nghệ thuật anime xu hướng với tỷ lệ phóng đại, cel shading và năng lượng hài hước.

**Xem trước:**
<img src="https://gptimagelib-kjuzvlsu.manus.space/manus-storage/da32948a-c45f-475d-b7cf-05fb5b0b3fbe_c8d4bc65.png" width="400" />

**Prompt:**
```
Create a trending anime art style image from the uploaded subject. Use confident line-work with slight variation and minimal cel shading using flat shadow shapes. Use bright, saturated colors and clean graphic lighting. The style is defined by exaggerated, cartoonish character proportions featuring highly expressive, simplistic facial features that allow for immense emotional range, with highly varied stretched anatomy. Transform the environment into a slightly warped space with playful perspective distortion and simplified objects. Composition and tone should be energetic, lively, and comedic in a fully stylized, non-realistic world.
```

[**→ Tạo ảnh với prompt này**](https://surgepix.ai/)

</details>

---

<details>
<summary><b>No.2 · Poster Bản Vẽ Kỹ Thuật</b></summary>

![EN](https://img.shields.io/badge/EN-blue?style=flat-square) ![OpenAI Official](https://img.shields.io/badge/OpenAI%20Official-orange?style=flat-square) ![Featured](https://img.shields.io/badge/%E2%AD%90%20Featured-yellow?style=flat-square)

**Mô tả:** Render bất kỳ đối tượng nào thành đường kẻ kỹ thuật trắng sạch trên nền xanh cobalt đậm với nhãn sơ đồ.

**Xem trước:**
<img src="https://gptimagelib-kjuzvlsu.manus.space/manus-storage/639cdd50-f091-4730-9257-870f89b5cda2_95a966a2.png" width="400" />

**Prompt:**
```
Create a single-subject blueprint poster using only the main subject from the uploaded image. Render it as a clean white technical line drawing on a deep cobalt-blue blueprint background with a subtle grid. Use flat 2D schematic linework, orthographic contours, construction lines, measurement ticks, arrows, and minimal technical labels. The composition may also include 2-3 smaller inset studies or alternate views. Keep the layout simple and balanced with no shading, no 3D, no lighting, no extra objects.
```

[**→ Tạo ảnh với prompt này**](https://surgepix.ai/)

</details>

---

<details>
<summary><b>No.3 · Chụp Ảnh Flash Ban Đêm</b></summary>

![EN](https://img.shields.io/badge/EN-blue?style=flat-square) ![OpenAI Official](https://img.shields.io/badge/OpenAI%20Official-orange?style=flat-square) ![Featured](https://img.shields.io/badge/%E2%AD%90%20Featured-yellow?style=flat-square)

**Mô tả:** Biến ảnh thành ảnh chụp editorial ban đêm candid với flash cứng, bóng tối sâu và thẩm mỹ thời trang-đêm thô ráp.

**Xem trước:**
<img src="https://gptimagelib-kjuzvlsu.manus.space/manus-storage/2bd35dac-4dab-4e1c-9cf3-530c9b5ed106_a23c4e53.png" width="400" />

**Prompt:**
```
Transform the photo into nighttime chic flash photography. Use harsh direct on-camera flash with strong highlights, deep shadows, and slight overexposure on the subject. Set the scene at night with a dark, moody background while keeping realistic colors and textures. Add a candid, editorial feel, slightly imperfect framing, motion energy, and spontaneous expression. Emphasize high contrast, glossy skin highlights, and subtle film grain for a raw, fashion-nightlife aesthetic.
```

[**→ Tạo ảnh với prompt này**](https://surgepix.ai/)

</details>

---

<details>
<summary><b>No.4 · Thước Phim Điện Ảnh</b></summary>

![EN](https://img.shields.io/badge/EN-blue?style=flat-square) ![OpenAI Official](https://img.shields.io/badge/OpenAI%20Official-orange?style=flat-square) ![Featured](https://img.shields.io/badge/%E2%AD%90%20Featured-yellow?style=flat-square)

**Mô tả:** Biến một ảnh thành 3 khung phim điện ảnh với hạt phim analog, mờ chuyển động và tone màu hoài cổ.

**Xem trước:**
<img src="https://gptimagelib-kjuzvlsu.manus.space/manus-storage/4_1ccaee0a.png" width="400" />

**Prompt:**
```
Transform the uploaded image into cinematic 3-frame sequential film stills (horizontal frames stacked vertically), full bleed edge-to-edge. Each frame should show a different moment from the same scene, with clear progression. Vary the composition, camera angle, and distances to create a sense of movement and storytelling. Use a cinematic, cooler-toned, high-contrast, deep-space blacks film with a natural color grade. Add subtle film grain, slight motion blur, and natural imperfections to emulate analog photography. Keep the composition candid and emotionally grounded, with a sense of movement and quiet storytelling. Overall aesthetic: cinematic, nostalgic, and organic, like raw film stills.
```

[**→ Tạo ảnh với prompt này**](https://surgepix.ai/)

</details>

---

<details>
<summary><b>No.5 · Lưới Thiết Kế Icon</b></summary>

![EN](https://img.shields.io/badge/EN-blue?style=flat-square) ![OpenAI Official](https://img.shields.io/badge/OpenAI%20Official-orange?style=flat-square) ![Featured](https://img.shields.io/badge/%E2%AD%90%20Featured-yellow?style=flat-square)

**Mô tả:** Trừu tượng hóa bất kỳ đối tượng nào thành 16–20 biến thể logo vector tối giản bao gồm hình học, line art, huy hiệu và chữ lồng.

**Xem trước:**
<img src="https://gptimagelib-kjuzvlsu.manus.space/manus-storage/2_e8adfc00.png" width="400" />

**Prompt:**
```
Transform this image into a grid of minimalist logos using the main subject as the core icon. Abstract and simplify the primary element into multiple unique vector-style logo marks. Each variation should reinterpret the same subject in different ways (geometric, line art, negative space, emblem, badge, monogram). Arrange 16-20 logos evenly on a light background. Keep designs clean, modern, with balanced spacing. Maintain consistency while exploring creative variations of the original subject as a cohesive branding logo collection.
```

[**→ Tạo ảnh với prompt này**](https://surgepix.ai/)

</details>

---

<details>
<summary><b>No.6 · Avatar Đồ Chơi 3D Designer</b></summary>

![EN](https://img.shields.io/badge/EN-blue?style=flat-square) ![OpenAI Official](https://img.shields.io/badge/OpenAI%20Official-orange?style=flat-square) ![Featured](https://img.shields.io/badge/%E2%AD%90%20Featured-yellow?style=flat-square)

**Mô tả:** Tạo render đồ chơi vinyl 3D bóng loáng cao cấp cho bất kỳ ai với tóc điêu khắc, kính mát retro và ánh sáng studio trên nền trời xanh.

**Xem trước:**
<img src="https://gptimagelib-kjuzvlsu.manus.space/manus-storage/8_83cb1c40.png" width="400" />

**Prompt:**
```
Create a premium glossy 3D "designer toy" render of the subject(s) using the uploaded image as the only reference. Render one floating head per person (no duplication), cropped cleanly below the jaw with a visible neck and full head comfortably framed. Style: high-quality vinyl figure with ultra-smooth, simplified forms, rounded volumes, and strong glossy reflections across key facial areas. Hair should be sculpted, glossy, and stylized, with embedded playful accessories. Include oversized retro wraparound sunglasses with vibrant, matching frame/lens colors. Use strong studio lighting with pronounced highlights. Background: blue sky with soft clouds.
```

[**→ Tạo ảnh với prompt này**](https://surgepix.ai/)

</details>

---

<details>
<summary><b>No.7 · Thiệp Ngày của Mẹ - Nữ Hoàng Cơ Đỏ</b></summary>

![EN](https://img.shields.io/badge/EN-blue?style=flat-square) ![Mother's Day](https://img.shields.io/badge/Mother's%20Day-ff69b4?style=flat-square) ![Featured](https://img.shields.io/badge/%E2%AD%90%20Featured-yellow?style=flat-square)

**Mô tả:** Tạo lá bài Nữ Hoàng Cơ Đỏ hai mặt phong cách gouache với khuôn mặt mẹ thay thế nữ hoàng — hoàn hảo cho món quà cá nhân hóa.

**Xem trước:**
<img src="https://ui-cos.tate-a-tate.com/surgepix-resources/gpt-image-2-prompts/55-mom-Chinese-AI.png" width="400" />

**Prompt:**
```
[Character reference image uploaded: mother's portrait] Mother's Day gouache greeting card, centered double-sided Queen of Hearts playing card design, retain the original queen's black and gold royal dress, crown, accessories, red heart decorations, light blush pink card base, texture brushstrokes, warm terracotta red handwritten lettering "You are the QUEEN of moms! Happy Mother's Day!", soft shadows, clean white background, cozy nostalgic vibe, high detail, soft focus, pastel color palette --ar 4:5 --style raw
The character's face and hairstyle are completely based on a real mother's gentle portrait, soft and kind facial features, natural and elegant real hairstyle from the mother's photo, naive and whimsical gouache art style
```

[**→ Tạo ảnh với prompt này**](https://surgepix.ai/)

</details>

---

<details>
<summary><b>No.8 · Hình Vẽ Đường Liên Tục Ngày của Mẹ</b></summary>

![EN](https://img.shields.io/badge/EN-blue?style=flat-square) ![Mother's Day](https://img.shields.io/badge/Mother's%20Day-ff69b4?style=flat-square) ![Featured](https://img.shields.io/badge/%E2%AD%90%20Featured-yellow?style=flat-square)

**Mô tả:** Chân dung mẹ và con một nét vẽ tối giản, kết hợp họa tiết hoa hồng và thông điệp viết tay thanh lịch trên nền kem mềm.

**Xem trước:**
<img src="https://ui-cos.tate-a-tate.com/surgepix-resources/gpt-image-2-prompts/Mother%27s%20Day%202026.png" width="400" />

**Prompt:**
```
Minimalist continuous line drawing. A soft pink color line forming a minimalist portrait of a mother embracing her child. Woven into the line is a simple rose flower design, all done in one fluid stroke. Light cream background with a soft pink wash. On the right side, elegant flowing penmanship writing the word "love". Below it, small delicate handwritten text: "You gave me roots to grow, and wings to fly — but most of all, you gave me a place where both are safe. Love you, Mom." Romantic, feminine, artistic, delicate line quality, minimalist art.
```

[**→ Tạo ảnh với prompt này**](https://surgepix.ai/)

</details>
---

## 🌍 Hỗ Trợ Đa Ngôn Ngữ

Thư viện hỗ trợ nhiều ngôn ngữ và sẽ tiếp tục bổ sung thêm:

- 🇺🇸 English (Tiếng Anh)
- 🇨🇳 中文 (Tiếng Trung)
- 🇵🇱 Polski (Tiếng Ba Lan)
- 🇻🇳 Tiếng Việt
- 🇮🇩 Bahasa Indonesia (Tiếng Indonesia)
- 🇹🇷 Türkçe (Tiếng Thổ Nhĩ Kỳ)
- ➕ Thêm ngôn ngữ sắp ra mắt

---

## 🚀 Cách Sử Dụng

**Cách 1 — Dùng website (khuyến nghị)**
1. Truy cập [surgepix.ai/resources/gpt-image-2-prompts](https://surgepix.ai/resources/vi/gpt-image-2-prompts)
2. Duyệt hoặc lọc theo danh mục
3. Nhấn **Sao chép** để lấy prompt, hoặc **Tạo ảnh** để tạo trực tiếp

**Cách 2 — Dùng trong ChatGPT / OpenAI API**
1. Sao chép bất kỳ prompt nào từ thư viện
2. Dán vào ChatGPT (cần bật GPT-Image-2) hoặc OpenAI Images API
3. Điều chỉnh tham số theo nhu cầu

---

## 🔗 Truy Nguồn Gốc

Các prompt cộng đồng đều có liên kết trực tiếp đến bài đăng gốc trên X (Twitter), Weibo, Reddit và các nền tảng khác — giúp bạn theo dõi nhà sáng tạo, hiểu ngữ cảnh và ghi nhận công lao đúng người.

---

## 📌 Về GPT-Image-2

GPT-Image-2 là mô hình tạo ảnh mới nhất của OpenAI, nổi bật với:

- Khả năng hiển thị văn bản trong ảnh xuất sắc
- Bố cục phức tạp nhiều thành phần
- Tuân theo chỉ dẫn chi tiết chính xác
- Poster, infographic, UI mockup và character sheet chất lượng cao

Tìm hiểu thêm: [Hướng dẫn GPT-Image-2 của SurgePix](https://surgepix.ai/blog/resources/chatgpt-images-2-0)

---

## 🛠️ Công Cụ Miễn Phí từ SurgePix

| Công cụ | Liên kết |
|---|---|
| 🤖 Tạo ảnh AI | [surgepix.ai](https://surgepix.ai) |
| 📊 Thư viện Prompt PPT | [Awesome PPT Prompts](https://surgepix.ai/resources/vi/awesome-ppt-prompts) |
| 🖼️ PDF sang JPG | [Công cụ](https://surgepix.ai/tools/operation?type=1) |
| ✂️ Thay đổi kích thước ảnh | [Công cụ](https://surgepix.ai/tools/operation?type=4) |
| 📺 Tạo banner YouTube | [Công cụ](https://surgepix.ai/tools/operation?type=5) |
| 💇 Kiểu tóc AI | [Công cụ](https://surgepix.ai/tools/operation?type=6) |
| 👨‍🍼 Ngày của Cha | [Công cụ](https://surgepix.ai/tools/operation?type=7) |
| 👩‍🍼 Ngày của Mẹ | [Công cụ](https://surgepix.ai/tools/operation?type=8) |
| 📊 Tạo biểu đồ cột | [Công cụ](https://surgepix.ai/tools/operation?type=9) |
| 🥧 Tạo biểu đồ tròn | [Công cụ](https://surgepix.ai/tools/operation?type=10) |
| 🔵 Sơ đồ Venn | [Công cụ](https://surgepix.ai/tools/operation?type=11) |

---

## 📬 Theo Dõi Chúng Tôi

- Theo dõi trên [X / Twitter](https://x.com/surgepix_AI)
- Tham gia [Discord](https://discord.gg/wC5XeHN6fR)
- Truy cập [surgepix.ai](https://surgepix.ai)

---

<div align="center">

Được tạo với ❤️ bởi [SurgePix](https://surgepix.ai) · Cập nhật hàng tuần · Miễn phí mãi mãi

</div>
